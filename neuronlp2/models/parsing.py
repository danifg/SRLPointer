...
class NewStackPtrNet(nn.Module):
	...
	#No se usa
	    ...
	    #exit(0)
            ...
            #Lo que estamos haciendo con la mascara es que si aparece un 0, entonces no lo tenemos en cuenta
            #Esto nos interesa cuando el nodo no apunta a nada, pero si apunta al root, entonces si que nos interesa
	    ...
	    src_encoding = src_encoding + output_enc_previous
            ...
            #print 'heads stack', heads_stack.data.t()
            #print 'previous heads', previous.data.t()
            #exit(0)
	...
	for t in range(num_step):
		...
		if stop[base_id] == True:
			...
			if child_id == head or num_heads[base_id]==max_num_heads-1:
				...
				new_positions[cc]=positions[base_id]
				new_stacked_heads[cc] = [stacked_heads[base_id][i] for i in range(len(stacked_heads[base_id]))]
				new_stacked_heads[cc].append(positions[base_id])#Mantenemos y repetimos la posicion en el stack
